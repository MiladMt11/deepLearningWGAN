...
class WGAN():
    ...
    def train(self, train_loader):
        try:
            os.mkdir('../checkpoint/WGAN_FashionMNIST/')
        except:
            pass
        try:
            self.load()
    ...
    def save(self):
        torch.save({"epoch": self.epoch,
                    ...
                    "Best FID score": self.best_fid}, "../checkpoint/WGAN_FashionMNIST/G.pth")
        torch.save({"D_state_dict": self.D.state_dict(),
                    "optimizer_D": self.optim_D.state_dict(),
                    "losses_fake": self.Fake_losses,
                    "losses_real": self.Real_losses}, "../checkpoint/WGAN_FashionMNIST/D.pth")
        if self.epoch == self.maxepochs:
            torch.save({"epoch": self.epoch,
                        ...
                        "losses_G": self.G_losses,
                        "FID scores": self.fid_score,
                        "Best FID score": self.best_fid}, "../checkpoint/WGAN_FashionMNIST/G_{}.pth")
            torch.save({"D_state_dict": self.D.state_dict(),
                        "optimizer_D": self.optim_D.state_dict(),
                        "losses_fake": self.Fake_losses,
                        "losses_real": self.Real_losses}, "../checkpoint/WGAN_FashionMNIST/D_{}.pth".format(self.epoch))
        print("model saved!")

    def load(self):
        checkpoint_G = torch.load("../checkpoint/WGAN_FashionMNIST/G.pth")
        checkpoint_D = torch.load("../checkpoint/WGAN_FashionMNIST/D.pth")
    ...
    def evaluate(self):
        ...
        with torch.no_grad():
            ...
            utils.save_image(grid, '../Results/WGAN_FashionMNIST/img_generatori_iter_{}.png'.format(self.epoch))
